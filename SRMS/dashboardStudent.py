...
class studentSystem:
    def __init__(self,home):
        self.home=home
        self.home.title("Student's Dashboard")
        self.home.geometry("1450x700+0+0")
        self.home.config(bg="white")

        #Title of Student Page
        title=Label(self.home,text="Student's Result",font=("times new roman",20,"bold"),bg="purple",fg="white").place(x=0,y=0,relwidth=1,height=50)

        #Searching Button
        self.var_search=StringVar()
        lbl_rollno = Label(self.home,text="Enter Reg. No. ",font=("times new roman",25,"bold"),bg="white").place(x=450,y=60)
        ...
        button_Logout=Button(text="Logout",font=("times new roman",15,"bold"),bg="red",fg="white",cursor="hand2",command=self.logout).place(x=1150,y=70,width=100,height=35)

        #Result Of Student and content to show
        lbl_roll = Label(self.home,text="Reg No.",font=("times new roman",15,"bold"),bg="white",bd=2,relief=GROOVE).place(x=100,y=200,width=190,height=90)
    ...
    def search(self):
        ...
        try:
            if self.var_search.get()=="":
                messagebox.showerror("Error","Reg No. is required!",parent=self.home)
            else:
